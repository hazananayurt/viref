...
def generation_test(args, save_path):
	...
	results_path = os.path.join('viref', 'generation_results.txt')
	...
	with torch.no_grad():
		for batch_idx, sampled_batch in enumerate(test_loader):
			...
			_, (hn, cn) = encoder(features, expanded_initial_scale_weights, expanded_h0, expanded_c0)
			...
			for i in range(decoder_input.shape[1]):
				...
				for log_probs, decoder_input_i, hi, ci, end_mask, sampled_words in top_probabilities:
					decoder_out_i, (hi, ci) = decoder(decoder_input_i, hi, ci)
					out = ff1(decoder_out_i)
					scale_weights = out[:, 0, :]
					_, (hn, _) = encoder(features, scale_weights, expanded_h0, expanded_c0)
					out_i = ff2(decoder_out_i[:, 0, :], hn)
